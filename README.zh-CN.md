# Website Screenshot

[English](README.md) | [简体中文](README.zh-CN.md) | [日本語](README.ja-JP.md) | [Deutsch](README.de-DE.md) | [한국어](README.ko-KR.md) | [Français](README.fr-FR.md) | [Português (Brasil)](README.pt-BR.md) | [العربية](README.ar-SA.md) | [Русский](README.ru-RU.md) | [Bahasa Melayu](README.ms-MY.md)

把网站、HTML 和移动端页面导出为 **JPG、PNG、WebP 或 PDF**。

**Website Screenshot** 是一套围绕真实用户需求打造的多语言截图与导出产品。无论你想做 **网站截图**、**网页截图**、**网页快照**、**URL 转图片**、**HTML 转图片**、**HTML 转 PDF**、**iPhone 截图预览**、**Android 截图预览**，还是 **整页截图**、**长网页截图**、**网页转 PDF**，这套项目都能覆盖。

## 为什么值得用

很多工具只能解决一个点状需求：

- 只能截网站，不能处理 HTML
- 只能导出图片，不能导出 PDF
- 只能看桌面端，不能看手机尺寸
- 只能截首屏，不能做整页截图

Website Screenshot 把这些相互关联的需求统一到一个产品里：

- 网站截图
- 网页截图
- 网页快照
- URL 转图片
- HTML 转图片
- HTML 转 PDF
- 网页转 PDF
- iPhone 截图
- Android 截图
- 整页截图
- 长网页截图
- 移动端整页截图

## 核心能力

- 输入网站 URL，导出 JPG、PNG、WebP 或 PDF
- 把 HTML 和 CSS 渲染成图片或 PDF
- 保存整页，而不是只保存首屏
- 按常见 iPhone 尺寸预览网页
- 按主流 Android 机型尺寸预览网页
- 保存长落地页、产品页、帮助页和博客页，用于 QA、评审和归档

## 核心页面

- `website-screenshot`
- `html-screenshot`
- `iphone-screenshot`
- `android-screenshot`
- `full-page-screenshot`
- `website-to-pdf`
- `html-to-pdf`
- `website-full-page-screenshot`
- `iphone-full-page-screenshot`
- `android-full-page-screenshot`

## 输出格式

- JPG：适合轻量分享
- PNG：适合 UI 细节和视觉 QA
- WebP：适合更现代的图片输出
- PDF：适合审批、归档、打印和文档流转

## 适合谁

- 设计师做响应式检查
- QA 团队做页面验证和留证
- 营销团队保存落地页素材
- 产品团队记录页面变化
- 开发者导出 HTML 模板和组件

## 机型覆盖

### iPhone

支持从 iPhone SE、XR、XS 到 iPhone 17 系列的多种尺寸，包括：

- Pro
- Pro Max
- Plus
- Air
- e

### Android

支持多个主流品牌和机型，包括：

- Samsung Galaxy S / Z
- Google Pixel
- Xiaomi
- Redmi
- POCO
- OnePlus
- OPPO
- vivo
- Nothing
- Motorola
- Honor
- Huawei

## SEO 价值

这个项目不只是一个工具前端，它本身也是一套围绕高意图搜索需求构建的多语言 SEO 工具站，覆盖了：

- 网站截图
- 网页截图
- 网页快照
- URL 转图片
- HTML 转图片
- HTML 转 PDF
- 网页转 PDF
- iPhone 截图
- Android 截图
- 整页截图
- 长网页截图
- 移动端整页截图

如果你想做产品型 SEO、工具型 SEO 或多语言自然流量承接，这个仓库本身就有参考价值。

## API 与本地启动

详见：

- [API_USAGE.md](/Users/ericwu/MinibuddyFolder/WebSite/40%20htmtoimage/htmltoimage-showcase/API_USAGE.md)

本地运行：

```bash
pnpm install
pnpm run build
PORT=3021 pnpm start
```
